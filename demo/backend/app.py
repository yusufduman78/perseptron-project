...
def csv_records(path: Path, limit: int | None = None) -> list[dict]:
    if not path.exists():
        return []
...
def ranking_leaderboard() -> list[dict]:
    if not RANKING_METRICS_PATH.exists():
        return []
    metrics = pd.read_csv(
        RANKING_METRICS_PATH,
        usecols=["model", "map_at_12", "precision_at_10", "recall_at_10"],
    )
    summary = (
        ...
        .sort_values(["map_at_12", "precision_at_10", "recall_at_10"], ascending=False)
    )
...
def health() -> dict:
    checkpoint_files = {
        ...
        "image_only_effnet_cnn": CNN_MODEL_PATH.exists(),
    }
    data_files = {
    ...
    result_files = {
        "classification_metrics": CLASSIFICATION_METRICS_PATH.exists(),
        "cnn_metrics": CNN_METRICS_PATH.exists(),
        "ranking_metrics": RANKING_METRICS_PATH.exists(),
        "ranking_summary": RANKING_SUMMARY_PATH.exists(),
        "shap_summary": SHAP_SUMMARY_PATH.exists(),
        "gradcam_examples": GRADCAM_DIR.exists() and any(GRADCAM_DIR.glob("*.png")),
    }
...
@app.get("/api/metrics")
def metrics() -> dict:
    summary_markdown = RANKING_SUMMARY_PATH.read_text(encoding="utf-8") if RANKING_SUMMARY_PATH.exists() else ""
    return {
        "classification": csv_records(CLASSIFICATION_METRICS_PATH),
        "image_only_cnn": csv_records(CNN_METRICS_PATH),
        "ranking": ranking_leaderboard(),
        "ranking_summary_markdown": summary_markdown,
        "notes": {
            "main_classification_comparison": "tabular_only vs image_only_effnet_cnn vs late_fusion",
            "ranking_comparison": "tabular_only vs image_history vs late_fusion",
            "hybrid_scope": "late_fusion_hybrid_* is a post-ranking/reranking experiment, not a main model.",
        },
    }
...
def explainability() -> dict:
    ...
    if GRADCAM_DIR.exists():
        gradcam_examples = [
            {
                "name": path.name,
                "image_url": f"/api/gradcam/{path.name}",
            }
            for path in sorted(GRADCAM_DIR.glob("*.png"))
        ]
    return {
        "shap_top_features": csv_records(SHAP_SUMMARY_PATH, limit=12),
        "gradcam_examples": gradcam_examples,
        "interpretation": {
            "tabular": "SHAP summary highlights customer and article metadata used by tabular models.",
            "image_only_cnn": "Grad-CAM examples come from the EfficientNet image-only baseline.",
            "demo": "Demo recommendation explanations use visual-neighbor and metadata-match chips.",
        },
    }


@app.get("/api/gradcam/{filename}")
def gradcam_image(filename: str) -> FileResponse:
    if "/" in filename or "\\" in filename or not filename.endswith(".png"):
        raise HTTPException(status_code=400, detail="Invalid Grad-CAM filename")
