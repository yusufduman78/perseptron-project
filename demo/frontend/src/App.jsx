...
const MODEL_META = {
  tabular_only: {
    ...
    metric: 'AUC 0.7841 / Best 0.7985',
  },
  image_history: {
    title: 'Image History',
    eyebrow: 'Visual history baseline',
    detail: 'Müşteri geçmişinden görsel stil profili; CNN baseline değildir.',
    metric: 'AUC 0.7480',
  },
  late_fusion: {
    title: 'Late Fusion',
    eyebrow: 'Main model',
    detail: 'Metadata ve görsel geçmiş sinyalini birlikte skorlar.',
    metric: 'AUC 0.8063 / Best 0.8121',
  },
}

const REASON_LABELS = {
  'Aynı ürün tipi': 'Ürün tipi uyumu',
  'Urun tipi uyumu': 'Ürün tipi uyumu',
  'Benzer giyim grubu': 'Benzer kategori',
  'Renk eşleşmesi': 'Renk uyumu',
  'Yakın görsel geçmiş': 'Görsel geçmiş',
  'Gorsel gecmis': 'Görsel geçmiş',
  'Görsel profil uyumu': 'Stil profili',
  'Popüler ürün': 'Popüler',
  'Aday havuzu seçimi': 'Aday seçimi',
  'Aday havuzu secimi': 'Aday seçimi',
}
...
function FinalEvidence({ metrics, explainability }) {
  const classification = metrics?.classification || []
  const cnn = metrics?.image_only_cnn?.[0]
  const ranking = metrics?.ranking || []
  const shapFeatures = explainability?.shap_top_features || []
  const gradcamExamples = explainability?.gradcam_examples || []
  const classificationRows = [
    { label: 'Tabular', row: classification.find((row) => row.model === 'tabular_only') },
    { label: 'Image-only CNN', row: cnn },
    { label: 'Late Fusion', row: classification.find((row) => row.model === 'late_fusion') },
  ]
  ...
  return (
    <section className="evidence-section">
      <div className="section-heading compact">
        <div>
          <p className="section-kicker">Final evidence</p>
          <h2>V2 Kaggle sonuçları</h2>
        </div>
        <span>Full run artifactlerinden okunur; hybrid sadece reranking deneyidir.</span>
      </div>
      ...
      <div className="evidence-grid">
        <article className="evidence-card">
          ...
          {classificationRows.map(({ label, row }) => (
            <div className="metric-row" key={label}>
              <span>{label}</span>
              <strong>{formatMetricValue(row?.auc_roc)}</strong>
            </div>
          ))}
        </article>
        ...
        <article className="evidence-card">
          ...
          <div className="metric-row leader">
            <span>{rankingLeader?.model || 'late_fusion'}</span>
            <strong>MAP@12 {formatMetricValue(rankingLeader?.map_at_12 ?? 0.001262)}</strong>
          </div>
          {ranking.slice(1, 4).map((row) => (
            <div className="metric-row" key={row.model}>
              <span>{row.model}</span>
              <strong>{formatMetricValue(row.map_at_12)}</strong>
            </div>
          ))}
        </article>
        ...
        <article className="evidence-card wide">
          ...
          {gradcamExamples[0] && (
            <img
              ...
              src={productImage(gradcamExamples[0].image_url)}
...
function App() {
  ...
  async function loadExplainability() {
    try {
      const response = await fetch(`${API_BASE}/api/explainability`)
      const data = await response.json()
  ...
  return (
    <main className="marketplace-app">
      ...
      <Recommendations
        ...
        recommendLoading={recommendLoading}
      />
      <FinalEvidence metrics={metrics} explainability={explainability} />
    </main>
  )
}
